    ...
    <section className="py-24 px-8 bg-slate-50 relative overflow-hidden">
      <div className="max-w-[1400px] mx-auto flex flex-col items-center text-center relative z-10">
        ...
        <h2 className="text-4xl md:text-5xl font-bold text-slate-900 max-w-3xl mb-16 leading-tight reveal delay-100">
          Beyond the <br />
          <span className="text-primary italic font-serif">Classroom</span>
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {featuresData.map((feature, index) => (
            <div
              key={index}
              className={`reveal group relative rounded-3xl overflow-hidden aspect-[4/5] shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer delay-${(index + 1) * 100}`}
            >
              {/* Background Image */}
              <img
                src={feature.image}
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end text-left">
                <h3 className="text-3xl font-bold text-white mb-3 font-serif translate-y-0 lg:translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  {feature.title}
                </h3>
